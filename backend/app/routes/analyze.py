...
async def analyze_recording(
    ...
    logger.info(
        f"[{request_id}] Analyzing recording for user {user_id}, surah={surah}, ayah={ayah}, mode={mode}"
    )
    ...
    try:
        ...
        logger.info(
            f"[{request_id}] Saved audio file: {original_filename} ({len(content)} bytes)"
        )
        ...
        logger.info(
            f"[{request_id}] Matched ayah: Surah {matched['surah']}, Ayah {matched['ayah']}"
        )
        ...
        logger.error(
            f"[{request_id}] Error analyzing recording: {str(e)}", exc_info=True
        )
...
async def quick_analyze(
    ...
    target_word_index: int = Form(
        ..., description="Index of word to focus on (0-based)"
    ),
    ...
    request_id = str(uuid.uuid4())
    logger.info(
        f"[{request_id}] Quick analyze: word {target_word_index}, error_type={error_type}"
    )
    ...
    try:
        ...
        logger.info(
            f"[{request_id}] Quick result: passed={passed}, confidence={confidence:.2f}"
        )
